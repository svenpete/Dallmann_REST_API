...
SELECT DEVICE.inventory_number,designation,serial_number,gurantee,note,category, TUEV.timestamp AS NEXT_TUEV, UVV.timestamp AS NEXT_UVV
FROM DEVICE
INNER JOIN BEACON
...
SELECT DEVICE.inventory_number,designation,serial_number,gurantee,note, longitude, latitude, timesstamp
FROM DEVICE
INNER JOIN BEACON
ON DEVICE.beacon_major = BEACON.major AND DEVICE.beacon_minor = BEACON.minor
...
SELECT loan_day,loan_end, WORKER.name, WORKER.surname, PROJECT.name AS Baustelle, inventory_number
FROM BORROWS
INNER JOIN PROJECT
